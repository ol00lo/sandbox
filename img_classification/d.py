...
def classify_with_threshold_class_index(inputs):
    x, threshold = inputs
    max_confidence = tf.reduce_max(x, axis=-1, keepdims=True)
...
print(f"Predicted Class Index: {predicted_class_index[0][0]}")
print(f"Predicted Confidence: {predicted_confidence[0][0]}")
...
a = loaded_model.predict(preprocess(preprocess_image("cat.jpeg")))
