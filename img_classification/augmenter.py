...
def display_images(selected_files, images, labels):
    images = images[:4]
    if len(images) == 1:
        image = images[0]
    elif len(images) == 2:
...
parser.add_argument('--img_size', type=int, default=128, help='Size for resizing images (default: 128)')  # Новый аргумент
...
while True:
    ...
    for i in range(0, len(image_files), nbatch):
        ...
        display_exit = display_images(selected_files, images_aug, labels)
        if display_exit: 
